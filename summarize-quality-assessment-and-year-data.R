# Load packages
packages_all = c("readxl", "stringr", "officer", "ggplot2", "gridExtra", "cowplot", "data.table")
...
stacked_barplot <- function(data, filename) {
    ...
    plot <- ggplot(data, aes(x = "", y = percentage, fill = rating)) +
        ...
        theme(
            ...
            legend.key = element_blank()  # Add this line to remove the border around the legends
        ) + ggtitle(filename)
    return(plot)
}

# create a bar plot for section ratings as per above
selection_bias_plot <- stacked_barplot(quality_assessment$selection_bias, "quality-assessment-selection-bias")
data_collection_methods_plot <- stacked_barplot(quality_assessment$data_collection_methods, "quality-assessment-data-collection-methods")
withdrawals_and_dropouts_plot <- stacked_barplot(quality_assessment$withdrawals_and_dropouts, "quality-assessment-withdrawals-and-dropouts")
preprocessing_plot <- stacked_barplot(quality_assessment$preprocessing, "quality-assessment-preprocessing")
trajectory_modelling_plot <- stacked_barplot(quality_assessment$trajectory_modelling, "quality-assessment-trajectory-modelling")
associated_risk_factors_and_outcomes_plot <- stacked_barplot(quality_assessment$associated_risk_factors_and_outcomes, "quality-assessment-associated-risk-factors-and-outcomes")
evaluation_and_reporting_of_results_plot <- stacked_barplot(quality_assessment$evaluation_and_reporting_of_results, "quality-assessment-evaluation-and-reporting-of-results")
secondary_ratings_plot <- stacked_barplot(secondary_ratings, "quality-assessment-secondary")
overall_rating_plot <- stacked_barplot(quality_assessment$overall_rating, "quality-assessment-overall")
...
plot <- ggplot2::ggplot(data = studies_per_year) +
    ...
    theme(
        ...
        axis.title.y = element_text(size = 13, color = "black", margin = margin(r = 13)),
        axis.text.x = element_text(size = 11, color = "black", margin = margin(t = 13)),
        axis.text.y = element_text(size = 11, color = "black", margin = margin(r = 13)),
...
# combine the plots (rating_plots) with gridExtra
# p_combined <- grid.arrange(
#     plot,
#     selection_bias_plot,
#     data_collection_methods_plot,
#     withdrawals_and_dropouts_plot,
#     preprocessing_plot,
#     trajectory_modelling_plot,
#     associated_risk_factors_and_outcomes_plot,
#     evaluation_and_reporting_of_results_plot,
#     secondary_ratings_plot,
#     ncol = 1,
#     top = "Main title"
# )
p_combined <- plot_grid(
    ...
    overall_rating_plot,
    secondary_ratings_plot,
    align = "v", nrow = 10, rel_heights = c(0.284090909, rep(0.0795454545, 9)))
ggsave(paste0('output/svg/', 'quality-assessment-ratings--with-year-data.svg'), p_combined, dpi = 300, height = 10.56, width = 7)